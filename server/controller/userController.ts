...
export const registrationUser = catchAsyncError(
  async (req: Request, res: Response, next: NextFunction) => {
    try {
      ...
      const activationToken = createJWT_token("10m", user, jwtActivationKey);
  ...
  try {
    ...
    if (avatar && user) {
      if (user.avatar?.public_id) {
        //  if have old image then delete old then update
        ...
        //  here we dont have image because of social auth
        const myClode = await cloudinary.v2.uploader.upload(avatar, {
          folder: "lmsCloude",
        });
        user.avatar = {
...
export const allCreaters = async (
  req: Request,
  res: Response,
  next: NextFunction
) => {
